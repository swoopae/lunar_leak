...
void hack::menu::controls::c_multicombobox::init ( const c_vector2 & pos , const c_vector2 & size ) {
	this->m_size.x = ( size.x - 44 );
}

void hack::menu::controls::c_multicombobox::think ( const c_vector2 & pos ) {
	...
	for ( int i = 0; i < std::min<int> ( this->m_members.size ( ) , 10 ); i++ ) {
		if ( input::key_released ( 1 ) && input::mouse_in_region ( popup_pos.x , popup_pos.y + 10 + ( text_size.y + 4 ) * i , popup_size.x , text_size.y ) ) {
			auto & value = CONFIG_GET ( bool , this->m_hashes.at ( this->m_scroll + i ) );
...
void hack::menu::controls::c_multicombobox::draw ( c_vector2 & pos ) {
	const auto text_size = hack::game::renderer::text_position ( this->m_name , hack::game::renderer::fonts::menu_font );
	...
	std::string str = STR ( "none" );
	bool first = true;

	for ( int i = 0; i < this->m_members.size ( ); i++ ) {
		auto & value = CONFIG_GET ( bool , this->m_hashes.at ( i ) );

		if ( value ) {
			if ( first ) {
				str = this->m_members.at ( i );
				first = false;
			} else {
				str.append ( STR ( ", " ) );
				str.append ( this->m_members.at ( i ) );
			}
		}
	}

	hack::game::renderer::filled_rectangle ( pos , c_vector2 ( this->m_size.x , text_size.y + 4 ) , c_color ( 54 , 54 , 54 , 130 ) );
	hack::game::renderer::text ( c_vector2 ( pos.x + ( this->m_size.x / 2 ) , pos.y + 2 ) , c_color ( 255 , 255 , 255 ) , str , hack::game::renderer::fonts::menu_font , true );
	...
	if ( m_hash == this->m_value_hash ) {
		...
		for ( int i = 0; i < std::min<int> ( this->m_members.size ( ) , 10 ); i++ ) {
			...
			layer_queue.push_back ( utils::draw_call_t ( c_vector2 ( popup_pos.x + ( popup_size.x / 2 ) , popup_pos.y + 5 + ( text_size.y + 4 ) * i ) , true , this->m_members.at ( this->m_scroll + i ) , hack::game::renderer::fonts::menu_font , !value ? c_color ( 255 , 255 , 255 , 255 ) : config::accent_color ) );
		}
	}
}